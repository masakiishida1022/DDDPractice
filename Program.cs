...
class Program
{
    static void Main()
    {
        ...
        {
            // CSVファイルのパスを指定
            string csvFilePath = @"C:\Users\masaki\OneDrive\ドキュメント\成果アクション\users.csv";
            ...
            foreach (string line in lines)
            {
                ...
                // フォルダのパスを指定
                string folderPath = Path.Combine(@"C:\Users\masaki\OneDrive\ドキュメント\成果アクション\", folderName);
    ...
    static string[] GetUsernamesFromCsv(string csvFilePath)
    {
        // CSVファイルからユーザ名を取得
        string[] lines = File.ReadAllLines(csvFilePath);
        string[] usernames = lines.Length > 0 ? lines[0].Split(',') : new string[0];

        // 最初の二つのユーザ名を取得
        string[] selectedUsernames = usernames.Length >= 3 ? new string[] { usernames[0], usernames[1], usernames[2] } : new string[0];
